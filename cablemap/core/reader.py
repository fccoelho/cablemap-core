...
_CONTENT_PATTERN = re.compile(r'(?:<code><pre>)(.+?)(?:</pre></code>)', re.DOTALL|re.UNICODE)
...
_LINK_PATTERN = re.compile(r'<a[^>]*>', re.UNICODE)
...
def parse_signed_by(content, canonicalize=True):
    ...
    if tmp_signers in (u'CONFIDENTIAL', u'UNCLASSIFIED'):
        return []
    if canonicalize:
        tmp_signers = tuple(re.split(r'\s+', tmp_signers.strip()))
...
_TAGS_PATTERN = re.compile(r'(?<!\()(?:TAGE?S+|TAG|TAS:|TABS|TGS|AGS:|^(?=EFIN,))(?!\n\nNOFORN)(?:[:,;\s]*)(.+)', re.IGNORECASE|re.UNICODE|re.MULTILINE)
_TAGS_CONT_PATTERN = re.compile(r'(?:\n)([a-zA-Z_-]+.+)', re.MULTILINE|re.UNICODE)
_TAGS_CLEANUP_PATTERN = re.compile(r'\s{5,}[^\n]+|(?:[,\s]+(?:(?:UN)CLASSIFIED|SECRET|PAGE|SUBJECT|E\.\s+O\.)[^\n]+)')
_TAGS_CONT_NEXT_LINE_PATTERN = re.compile(r'[ ]*\n[ ]*[A-Za-z_-]+[ ]*,')
_TAG_PATTERN = re.compile(r'(GOI[ ]+(?:EX|IN)TERNAL)'
                          r'|(POLITICAL[ ]+PARTIES)'
                          r'|(USEU[ ]+BRUSSELS)|(POLITICS[ ]+FOREIGN[ ]+POLICY)'
                          r'|(MILITARY[ ]+RELATIONS)|(ISRAEL[ ]+RELATIONS)'
                          r'|(COUNTRY\s+CLEARANCE)|(CROS[ ]+GERARD)'
                          r'|(FOREIGN\s+\w+)|(MEDIA\s+REACTION\s+REPORT)'
                          r'|(ROOD[ ]+JOHN)|(NEW[ ]+ZEALAND)'
                          r'|(TIP\s+IN\s+\w+)'
                          r'|(MEETINGS[ ]+WITH[ ]+\w+)'
                          r'|(DOMESTIC[ ]+POLITICS)|(ITALIAN[ ]+POLITICS)'
                          r'|(ITALY[ ]+NATIONAL[ ]+ELECTIONS)|(IRAQI[ ]+FREEDOM)'
                          r'|(GLOBAL[ ]+DEFENSE)'
                          r'|(NOVO[ ]+GUILLERMO)|(REMON[ ]+PEDRO)'
                          r'|(JIMENEZ[ ]+GASPAR)|(CONSULAR[ ]+AFFAIRS)'
                          r'|(ECONOMIC[ ]+AFFAIRS)|(HUMAN[ ]+RIGHTS)'
                          r'|(BUSH[ ]+GEORGE)|(CARSON[ ]+JOHNNIE)|(ZOELLICK[ ]+ROBERT)'
                          r'|(GAZA[ ]+DISENGAGEMENT)|(ISRAELI[ ]+PALESTINIAN[ ]+AFFAIRS)'
                          r'|(COUNTER[ ]+TERRORISM)|(CLINTON[ ]+HILLARY)'
                          r'|(STEINBERG[ ]+JAMES)|(BIDEN[ ]+JOSEPH)|(RICE[ ]+CONDOLEEZZA)'
                          r'|(\w+\s+AND\s+\w+)'
                          r'|([A-Z_]+(?:[\-\w/]+)*)'
                          r'|(\([^\)]+\))'
                          r'|(?:,[ ]+)([A-Z_-]+[\-\s]{1,3}[A-Z_-]+(?:\s{1,2}[A-Z]+)?)', re.UNICODE|re.IGNORECASE)
...
def parse_tags(content, reference_id=None, canonicalize=True):
    ...
    if tags.endswith(',') or tags.endswith(', ') or _TAGS_CONT_NEXT_LINE_PATTERN.match(content, min_idx, max_idx):
        ...
        if m2:
            tags = re.sub(r'\s+', u' ', u' '.join([tags, _TAGS_CLEANUP_PATTERN.sub(u' ', m2.group(1))]))
    res = []
    if not canonicalize:
...
_START_SUMMARY_PATTERN = re.compile(r'(SUMMAR?Y( AND COMMENT)?( AND ACTION REQUEST)?( AND INTRODUCTION)?( AND TABLE OF CONTENTS)?[ ‐\-\n:\.]*)', re.IGNORECASE)
